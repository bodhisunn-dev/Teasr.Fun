...
export default function Messages() {
  ...
  // Get list of users who have paid for current user's content
  const { data: paidUsers = [] } = useQuery<UserType[]>({
    queryKey: ['/api/users/paid-for-content', address],
    enabled: !!address,
    queryFn: async () => {
  ...
  useEffect(() => {
    if (selectedConversation && address) {
      apiRequest(`/api/messages/${selectedConversation.id}/read`, {
        method: 'PUT',
      }).then(() => {
  ...
  return (
    <>
      ...
      >
        {/* Conversations and Paid Users List */}
                ...
                {conversations.length === 0 ? (
                  ...
                  conversations.map((conv: any, index: number) => (
                    <motion.div
                      ...
                      className={`p-3 sm:p-4 border-b cursor-pointer hover-elevate active-elevate-2 transition-all touch-manipulation ${
                        selectedConversation?.id === conv.user.id ? 'bg-accent/50' : ''
                      }`}
            ...
            <TabsContent value="paid-users" className="h-[calc(100%-40px)] m-0">
              <ScrollArea className="h-full">
                {paidUsers.length === 0 ? (
                  ...
                  paidUsers.map((user: UserType, index: number) => (
                    <motion.div
                      key={user.id}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: index * 0.05 }}
                      className={`p-3 sm:p-4 border-b cursor-pointer hover-elevate active-elevate-2 transition-all touch-manipulation ${
                        selectedConversation?.id === user.id ? 'bg-accent/50' : ''
                      }`}
          ...
          <Card className={`${!selectedConversation ? 'hidden md:block' : ''} md:col-span-2 overflow-hidden`}>
            {selectedConversation ? (
              ...
              <motion.div 
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="h-full flex items-center justify-center text-muted-foreground p-8 text-center"
              >
                <div>
